...
def batch_detect(image_dir, output_dir):
    ...
    img_filelist = [f for f in img_filelist if any(f.lower().endswith(ext) for ext in image_extensions)]
...
def make_sky_mask(src_image, border, type):
    ...
    if type == 1:
        ...
        assert type == 0 or type == 1,'type参数必须为0或1'
